export async function handler(payload: any = {}, __innerSDK__: any) {
  const { aptos } = __innerSDK;
...
async function genVerificationMethods(
  client: any,
  handle: string,
  syntax: string,
  key: string,
...
function verify(signature: string) {
  if (signature == "0x") {
    return false;
  }
  return true;
